...
unsigned int keyExpansion(u_int8_t*, u_int32_t*, unsigned int);
void testArithmetic();
void testSubAndRot();
...
unsigned int rotWord(unsigned int x) {
    return (x << 8) | (x >> 24);
} 
...
unsigned int keyExpansion(u_int8_t key[4*keyLength], u_int32_t word[blockSize*(numRounds+1)], unsigned int keyLength) {
    //TODO: check copilot
